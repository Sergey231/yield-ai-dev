...
import { buildDepositAssetToSubaccountPayload } from '@/lib/protocols/decibel/depositToSubaccount';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
interface DecibelVaultShareDepositModalProps {
  isOpen: boolean;
  onClose: () => void;
  vaultName: string;
  subaccountAddr: string;
  assetMetadataAddr: string;
  walletShareBalanceBaseUnits: bigint | number | string;
  shareSymbol?: string;
  sharePriceUsd?: number;
  isTestnet?: boolean;
}

export function DecibelVaultShareDepositModal({
  isOpen,
  onClose,
  vaultName,
  subaccountAddr,
  assetMetadataAddr,
  walletShareBalanceBaseUnits,
  shareSymbol = 'shares',
  sharePriceUsd = 0,
  isTestnet = false,
}: DecibelVaultShareDepositModalProps) {
  ...
  const walletShareBalance = useMemo(
    () => coerceBaseUnits(walletShareBalanceBaseUnits),
    [walletShareBalanceBaseUnits]
  );

  const { amount, amountString, setAmountFromString, setHalf, setMax, isValid } = useAmountInput({
    balance: walletShareBalance,
  ...
  const handleDeposit = async () => {
    ...
    try {
      const payload = buildDepositAssetToSubaccountPayload({
        subaccountAddr,
        assetMetadataAddr,
      ...
      toast({
        title: `${shareSymbol} deposit submitted`,
        description: txHash
          ? `Transaction ${txHash.slice(0, 6)}...${txHash.slice(-4)}. Shares will move from your Aptos wallet back into Decibel.`
          : 'Transaction submitted. Shares will move from your Aptos wallet back into Decibel.',
        action: txHash ? (
          <ToastAction
  ...
  const hasWalletShares = walletShareBalance > BigInt(0);
      ...
      <DialogContent className="w-full min-w-0 max-w-[min(100vw-2rem,425px)] overflow-x-hidden rounded-2xl p-6 sm:max-w-[425px] [&>button:last-child]:right-3 [&>button:last-child]:top-3 [&>button:last-child]:size-7 [&>button:last-child>svg]:size-3.5 sm:[&>button:last-child]:right-5 sm:[&>button:last-child]:top-5 [&>button:last-child]:transition-colors [&>button:last-child]:hover:bg-muted/40">
        ...
        <div className="grid min-w-0 gap-4 py-4">
          <TokenAmountInput
            ...
            availableText={`${formatTokenAmount(walletShareBalance, SHARE_DECIMALS)} ${shareSymbol}`}
            inputRef={amountInputRef}
            onHalf={setHalf}
            onMax={setMax}
            isOverBalance={amount > walletShareBalance}
          />

          {amount > walletShareBalance && (
            <p className="text-sm text-red-500">Amount exceeds your wallet share balance.</p>
          )}
