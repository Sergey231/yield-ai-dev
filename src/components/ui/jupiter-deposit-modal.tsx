...
interface JupiterDepositModalProps {
  ...
  isLoading?: boolean;
  /** Default: "Deposit to {symbol} Jupiter" */
  title?: string;
  /** Optional description under the title (only shown when set). */
  description?: string;
...
export function JupiterDepositModal({
  ...
  const dialogTitle = title ?? `Deposit to ${token.symbol} Jupiter`;
      ...
      <DialogContent className="w-full min-w-0 max-w-[min(100vw-2rem,425px)] overflow-x-hidden rounded-2xl p-6 sm:max-w-[425px] [&>button:last-child]:right-5 [&>button:last-child]:top-5 [&>button:last-child]:size-7 [&>button:last-child>svg]:size-3.5 [&>button:last-child]:transition-colors [&>button:last-child]:hover:bg-muted/40">
        <DialogHeader className="min-w-0 pr-11 sm:pr-12">
          <div className="flex min-w-0 items-center gap-2">
            <Image
              src={protocol?.logoUrl || "/protocol_ico/jupiter.png"}
              alt={protocol?.name || "Jupiter"}
              width={24}
              height={24}
