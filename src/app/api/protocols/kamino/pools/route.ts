...
let cache: KaminoPoolsCache | null = null;
let inFlight: Promise<KaminoPoolsCache> | null = null;
...
export async function GET() {
  try {
    ...
    if (inFlight) {
      // Prefer fast stale response while a refresh is running.
    ...
    inFlight = (async (): Promise<KaminoPoolsCache> => {
    ...
    if (!vaultsRes.ok) {
      ...
      const payload = { success: true as const, data: [] as InvestmentData[], count: 0 };
      const nextCache: KaminoPoolsCache = { atMs: Date.now(), payload };
      cache = nextCache;
      return nextCache;
    }
    ...
    // Limit concurrency to avoid rate limits but keep UI responsive.
    const CONCURRENCY = 6;
    for (let i = 0; i < candidates.length; i += CONCURRENCY) {
      ...
      const rows = await Promise.all(
        chunk.map(async (v) => {
          ...
          const vaultName = String(v.state?.name ?? "").trim();
          const tokenMint = String(v.state?.tokenMint ?? "").trim();
          if (!vaultAddress || !vaultName || !tokenMint) return null;
